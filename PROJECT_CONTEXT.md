...
> Status per: 1 Agustus 2026 (termasuk restrukturisasi SPEC_FOKUS §1/§2/§4).
...
**Kanal Telegram + proaktif (17 Agu 2026):** bot @Koppilot_bot LIVE — webhook `/api/telegram` (Vercel Edge, balas-200-lalu-waitUntil, typing indicator, markdown Telegram). Otak AI dipisah ke `server/otak.ts` (`jawabPendamping` non-streaming, tools sama). **Ringkasan Pagi proaktif**: `server/digest.ts` (deterministik dari data, bukan LLM) dikirim cron Vercel 06:00 WIB (`vercel.json` crons → `/api/digest`, otorisasi Bearer CRON_SECRET / `?rahasia=TELEGRAM_SECRET`, `?pratinjau=1` untuk lihat isinya) ke `TELEGRAM_CHAT_IDS`; perintah bot: `/start`, `/langganan` (lihat chat id), `/ringkasan`. Env: TELEGRAM_BOT_TOKEN, TELEGRAM_SECRET, TELEGRAM_CHAT_IDS, CRON_SECRET. **BNI MOVE** (kredit UMKM digital — produk riil BNI) masuk `layananBni` + fitur anggota "Ajukan Pinjaman" di profil (dialog mock via BNI MOVE). Kabar Pasar punya sumber "BNI Economic Outlook". Sapaan/insight diselaraskan ke angka data (penjualan kemarin Rp6,3 jt).

**BNI sebagai jalur utama ekspor (16 Agu 2026):** halaman Kesiapan punya hero card "BNI Xpora — Direkomendasikan" (pola recommended-tier: border tint, benefit chips, CTA "Ajukan lewat BNI Xpora" → dialog profil pengajuan mock berisi skor/komoditas/relasi BNI → status terkirim); program pemerintah diturunkan jadi "pelengkap". BNI juga di point of need langkah lain: strip "Pembiayaan ekspor BNI · Segera" di kartu komoditas potensi tinggi (Peluang) dan kartu "Trade Finance & LC BNI" (Dokumen). Semua berlabel mock/Segera — bukan klaim enrollment.

**Data pasar eksternal mock (15 Agu 2026):** `mocks/pasar.ts` — kurs valas USD/JPY/SGD→IDR + perubahan bulanan, tren harga arabika dunia 12 bulan (sparkline, +22% setahun), dan 4 "Kabar Pasar" terkurasi bertanggal+sumber+badge dampak (Reuters/Kemendag/KKP/BPS — angka contoh, bukan riil; disclaimer di UI). Tampil sebagai panel kolom kanan di `/ekspor/peluang`; ikut masuk `dataPeluangEkspor()` sehingga AI bisa menarasikan kurs/tren/berita (teruji live). Menjawab poin mentor "integrasi data lebih luas" untuk segmen ekspor — versi API riil tetap ditunda.
...
├── src/components/
│   ├── layout/AppShell.tsx          # sidebar (grup Copilot ala deck) + pemilih peran + topbar + footer BNI
